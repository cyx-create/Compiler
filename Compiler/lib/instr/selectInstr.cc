...
struct SelectionContext {
    ...
    const std::set<int> *funcDefinedTemps = nullptr;
};
...
static int resolveVersionedTempNum(
    int tempNum,
    ...
    int logical = tempNum >= 10000 ? tempNum / 100 : tempNum;
    auto search = [&](const std::set<int> &defs) -> int {
        for (int ver = 99; ver >= 0; --ver) {
            int candidate = logical * 100 + ver;
            if (defs.count(candidate) > 0) {
                return candidate;
            }
        }
        if (defs.count(tempNum) > 0) {
            return tempNum;
        }
        return -1;
    };
    int resolved = search(definedTemps);
    if (resolved >= 0) {
        return resolved;
    }
    if (funcDefinedTemps != nullptr) {
        resolved = search(*funcDefinedTemps);
        if (resolved >= 0) {
            return resolved;
        }
    }
...
static std::set<int> collectDefinedTempsInFunc(
...
static tree::Temp *resolvedTemp(
    int tempNum,
    const std::set<int> &definedTemps,
    const std::set<int> *funcDefinedTemps = nullptr
) {
    return makeTemp(
        resolveVersionedTempNum(tempNum, definedTemps, funcDefinedTemps)
    );
}
...
static void emitLoadWithChain(
    ...
    int baseNum = resolveVersionedTempNum(
        ptrCalcBaseTempNum(ptrCalc), definedTemps, funcDefinedTemps);
    ...
    int offNum = resolveVersionedTempNum(
        ptrCalcOffsetTempNum(ptrCalc), definedTemps, funcDefinedTemps);
    appendOper(
        bundle,
        "ldr `d0, [`s0, `s1]",
        {dst},
    ...
    switch (stm->kind) {
        case quad::QuadKind::LOAD: {
            ...
            if (chainUsed >= 0) {
                ...
                auto *ptrCalc = chainNode == nullptr
                                    ...
                                    : dynamic_cast<const quad::QuadPtrCalc *>(
                                          chainNode->quadStatement);
                if (ptrCalc != nullptr) {
                    int ptrTempNum = resolveVersionedTempNum(
                        ptrCalcDstTempNum(ptrCalc),
                        definedTempsAtBuild
                    );
            ...
            if (load->src->kind == quad::QuadTermKind::TEMP) {
                ...
                tree::Temp *src = resolvedTemp(
                    load->src->get_temp()->temp->num,
                    definedTempsAtBuild,
                    ctx.funcDefinedTemps
                );
        ...
        case quad::QuadKind::PTR_CALC: {
            ...
            emitPtrCalcBundle(
                bundle, ptrCalc, definedTempsAtBuild, ctx.funcDefinedTemps);
        ...
        case quad::QuadKind::MOVE_BINOP: {
            ...
            if (leftNum >= 0) {
                leftNum = resolveVersionedTempNum(
                    leftNum, definedTempsAtBuild, ctx.funcDefinedTemps);
            }
            if (rightNum >= 0) {
                rightNum = resolveVersionedTempNum(
                    rightNum, definedTempsAtBuild, ctx.funcDefinedTemps);
            }
            ...
            if (binop->binop == "+" && rightConst >= 0 && leftNum >= 0) {
                ...
                int value = leftConst + rightConst;
                if (fitsMovImmediate(value)) {
                    appendOper(
                        bundle,
                        "mov `d0, #" + std::to_string(value),
        ...
        case quad::QuadKind::MOVE_EXTCALL: {
            ...
            appendOper(bundle, "mov `d0, `s0", {dst}, {makeTemp(0)});
            return bundle;
        }
        ...
        case quad::QuadKind::MOVE_CALL: {
            ...
            if (call->obj_term != nullptr &&
                call->obj_term->kind == quad::QuadTermKind::TEMP) {
                tree::Temp *target = resolvedTemp(
                    call->obj_term->get_temp()->temp->num,
                    definedTempsAtBuild,
                    ctx.funcDefinedTemps
                );
                appendOper(bundle, "mov r12, `s0", {}, {target});
            }
...
static bool earlierBlockStatementsScheduled(
    size_t index,
    const std::vector<bool> &scheduled
) {
    for (size_t j = 0; j < index; ++j) {
...
static bool mustPreserveProgramOrder(const quad::QuadStm *stm) {
    if (stm == nullptr) {
        return false;
    }
    return stm->kind == quad::QuadKind::EXTCALL ||
           stm->kind == quad::QuadKind::MOVE_EXTCALL ||
           stm->kind == quad::QuadKind::CALL ||
           stm->kind == quad::QuadKind::MOVE_CALL;
}

static bool chainAccessOperandsReady(
    const advDFG &graph,
    const advDFGNode *node,
    const std::set<int> &definedTemps
) {
    if (node == nullptr || node->chainUsed < 0) {
        return true;
    }
    const advDFGNode *chainNode = findChainDefNode(graph, node->chainUsed);
    if (chainNode == nullptr || chainNode->quadStatement == nullptr) {
        return true;
    }
    auto *ptrCalc = dynamic_cast<const quad::QuadPtrCalc *>(
        chainNode->quadStatement);
    ...
    int baseNum = resolveVersionedTempNum(ptrCalcBaseTempNum(ptrCalc), definedTemps);
    if (baseNum >= 0 && definedTemps.count(baseNum) == 0 &&
        definedTemps.count(ptrCalcBaseTempNum(ptrCalc)) == 0) {
        return false;
    }
    if (ptrCalcConstOffset(ptrCalc) < 0) {
        int offNum = resolveVersionedTempNum(ptrCalcOffsetTempNum(ptrCalc), definedTemps);
        if (offNum >= 0 && definedTemps.count(offNum) == 0 &&
            definedTemps.count(ptrCalcOffsetTempNum(ptrCalc)) == 0) {
            return false;
        }
    }
    return true;
...
void selectInstructionsForBlock(
    ...
    int &nextTempNum,
    const std::set<int> &funcDefinedTemps
) {
    if (blockGraph.graph.getNodes().empty()) {
        return;
    }

    SelectionContext ctx{nextTempNum, blockGraph.graph, {}, &funcDefinedTemps};
    ...
    while (scheduledCount < nodes.size()) {
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
            ...
            if ((probe.stm != nullptr &&
                 (probe.stm->kind == quad::QuadKind::LOAD ||
                  probe.stm->kind == quad::QuadKind::STORE)) &&
                !chainAccessOperandsReady(
                    blockGraph.graph, nodes[i], definedTemps)) {
                continue;
            }
            if (!earlierBlockStatementsScheduled(i, scheduled)) {
                continue;
            }
            ...
            if (bestIndex < 0) {
                ...
                if (candidateExt && bestExt) {
                    better = quadIndex < bestQuadIndex;
                } else if (candidateExt && bestConst &&
        ...
        if (bestIndex < 0) {
            for (size_t i = 0; i < nodes.size(); ++i) {
                if (!scheduled[i] &&
                    earlierBlockStatementsScheduled(i, scheduled)) {
                    bestIndex = static_cast<int>(i);
                    break;
                }
            }
...
void runInstructionSelectionPass(
    ...
    for (size_t funcIndex = 0; funcIndex < funcCount; ++funcIndex) {
        ...
        std::set<int> funcDefinedTemps =
            collectDefinedTempsInFunc(funcSchedule->quadFunc);
        ...
        for (size_t blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
            ...
            selectInstructionsForBlock(
                *blockGraph, *blockSchedule, nextTempNum, funcDefinedTemps);
        }
