...
static int resolveVersionedTempNum(
    ...
    for (int ver = 99; ver >= 0; --ver) {
        ...
        if (definedTemps.count(candidate) > 0) {
            return candidate;
        }
    }
    if (definedTemps.count(tempNum) > 0) {
        return tempNum;
    }
    return tempNum;
}
...
static void appendParamMoves(
    ...
    static const int argRegNums[] = {0, 1, 2, 3};
...
static bool falseDirectlyBranchesToTrue(
    const quad::QuadFuncDecl *func,
    const quad::QuadBlock *falseBlock,
    const quad::QuadBlock *trueBlock
) {
    if (falseBlock == nullptr || trueBlock == nullptr ||
        falseBlock->quadlist == nullptr || falseBlock->quadlist->empty()) {
        return false;
    }

    const quad::QuadStm *last = falseBlock->quadlist->back();
    if (last == nullptr) {
        return false;
    }
    ...
    if (last->kind == quad::QuadKind::CJUMP) {
        ...
        return blockByLabel(func, cjump->t) == trueBlock ||
               blockByLabel(func, cjump->f) == trueBlock;
    }
    ...
    if (last->kind == quad::QuadKind::JUMP) {
        ...
        return blockByLabel(func, jump->label) == trueBlock;
    }
...
static std::string invertRelop(const std::string &relop) {
    if (relop == ">") return "<=";
    if (relop == ">=") return "<";
    if (relop == "<") return ">=";
    if (relop == "<=") return ">";
...
static void emitCompareAndBranch(
    ...
    if (left != nullptr && right != nullptr && branchTarget != nullptr) {
...
static void emitReturn(
    ...
    if (ret != nullptr && ret->exp != nullptr) {
        ...
        if (value != nullptr) {
            ...
            appendOper(funcSchedule, "mov r0, #0");
        }
    } else {
        appendOper(funcSchedule, "mov r0, #0");
    }
...
static tree::Label *bodyLabelForBlock(
    ...
    auto it = ctx.blockBodyLabels.find(block);
    if (it != ctx.blockBodyLabels.end()) {
        return it->second;
    }
    return block->entry_label;
}

static tree::Label *freshLayoutLabel(int &nextLayoutLabelNum) {
    return new tree::Label(nextLayoutLabelNum++);
}
...
static void layoutFunction(
    ...
    int nextLayoutLabelNum = func->last_label_num + 100000;
    ctx.nextLayoutLabelNum = &nextLayoutLabelNum;
    ctx.definedTemps = collectDefinedTempsInFunc(func);
    ...
    ctx.emitBlock = [&](const quad::QuadBlock *block, tree::Label *predLabel) {
        ...
        if (blockSchedule != nullptr && !blockSchedule->phiFunctions.empty()) {
            tree::Label *bodyLabel = freshLayoutLabel(*ctx.nextLayoutLabelNum);
            ctx.blockBodyLabels[block] = bodyLabel;
            appendLabel(funcSchedule, bodyLabel);
        }
        ...
        if (last == nullptr) {
            emitReturn(funcSchedule, nextTempNum, nullptr, ctx.definedTemps);
            return;
        }
        ...
        if (isExitCallStmt(last)) {
            emitExitCall(funcSchedule, nextTempNum, last, ctx.definedTemps);
            return;
        }
        ...
        if (last->kind == quad::QuadKind::CJUMP) {
            ...
            preScheduleBlock *trueSchedule =
                scheduleForBlock(ctx.funcPreSchedule, trueBlock);

            const bool falseCanReachTrue =
                falseDirectlyBranchesToTrue(func, falseBlock, trueBlock);
            tree::Label *branchTarget = cjump->t;
            tree::Label *trueTrampoline = nullptr;
            if (falseCanReachTrue) {
                trueTrampoline = freshLayoutLabel(*ctx.nextLayoutLabelNum);
                branchTarget = trueTrampoline;
            }

            // Branch to the true target (or trampoline); fall through into false.
            ...
            if (falseCanReachTrue) {
                appendLabel(funcSchedule, trueTrampoline);
                emitPhiCopiesForEdge(
                    funcSchedule,
                    trueSchedule,
                    block->entry_label
                );
                if (ctx.visited.count(trueBlock)) {
                    tree::Label *dest = bodyLabelForBlock(ctx, trueBlock);
                    if (dest != nullptr) {
                        appendOper(
                            funcSchedule,
                            "b `j0",
                    ...
                    ctx.emitBlock(trueBlock, nullptr);
                }
            } else {
        ...
        if (last->kind == quad::QuadKind::JUMP) {
            ...
            if (targetBlock != nullptr && !ctx.visited.count(targetBlock)) {
                ...
                tree::Label *dest = bodyLabelForBlock(ctx, targetBlock);
                if (dest != nullptr) {
                    appendOper(
                        funcSchedule,
                        "b `j0",
