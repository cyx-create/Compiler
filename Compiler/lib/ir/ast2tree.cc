...
static bool irEndsWithReturn(tree::Stm *stm) {
    if (stm == nullptr) {
        return false;
    }
    ...
    auto *seq = dynamic_cast<tree::Seq *>(stm);
    if (seq == nullptr || seq->sl == nullptr || seq->sl->empty()) {
        return false;
    }
    return dynamic_cast<tree::Return *>(seq->sl->back()) != nullptr;
}

static tree::Stm *appendImplicitReturn(tree::Stm *body) {
    if (irEndsWithReturn(body)) {
        return body;
    }
    auto *ret = new tree::Return(new tree::Const(0));
    if (body == nullptr) {
        return ret;
    }
    auto *seq = dynamic_cast<tree::Seq *>(body);
    if (seq != nullptr) {
        if (seq->sl == nullptr) {
            seq->sl = new vector<tree::Stm *>();
        }
        seq->sl->push_back(ret);
        return seq;
    }
...
if (node->vdl != nullptr) {
    ...
    body_seq = appendImplicitReturn(body_seq);
...
void ASTToTreeVisitor::visit(fdmj::MethodDecl* node) {
    ...
    tree::Stm *body_seq = appendImplicitReturn(new tree::Seq(all_stms));
