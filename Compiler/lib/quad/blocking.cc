...
static vector<QuadBlock*>* split_block(QuadBlock* block, tree::Temp_map * temp_map);
...
QuadProgram *blocking(QuadProgram *program) {
    ...
    QuadProgram* new_program = new QuadProgram(
        new_func_decl_list,
        program->last_label_num,
        program->last_temp_num
    );
...
static QuadFuncDecl* blocking(QuadFuncDecl* func_decl) {
    ...
    tree::Temp_map * temp_map = new tree::Temp_map();
        ...
        if (stmt->kind == QuadKind::LABEL) {
            ...
            if (label_stmt && label_stmt->label) {
                ...
                if (!current_stmts->empty()) {
                    ...
                    QuadJump* jump = new QuadJump(label_stmt->label, empty_def, empty_use);
                    ...
                    QuadBlock* new_block = new QuadBlock(current_stmts, current_label, exit_labels);
