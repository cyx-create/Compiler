...
static void pushDefForOrig(int logical, QuadType ty, map<int, vector<int>>& stacks,
                           vector<int>& popOrder, int& maxTemp, int blockNum) {
    ...
    if (vn > maxTemp)
        maxTemp = vn;
    if (ver >= 1)
        g_diag.createdVersionBlocksByVar[logical][ver].insert(blockNum);
}

static void renameStatement(QuadStm* stm, map<int, vector<int>>& stacks, vector<int>& myPops,
                            int& maxTemp, int blockNum);
...
static void renameBlock(int blockNum, QuadFuncDecl* func, ControlFlowInfo* cfi,
    ...
    for (auto* stm : *block->quadlist) {
        ...
        int logical = logicalTempNum(phi->temp_exp->temp->num);
        QuadType ty = phi->temp_exp->type;
        pushDefForOrig(logical, ty, stacks, myPops, maxTemp, blockNum);
    ...
    for (auto* stm : *block->quadlist) {
        if (stm->kind == QuadKind::LABEL || stm->kind == QuadKind::PHI)
            continue;
        renameStatement(stm, stacks, myPops, maxTemp, blockNum);
    }
...
// Order functions for SSA diagnostic printing (generally follows XML/source order).
static void sortFuncFlowForDiag(vector<FuncFlowInfo*>& ordered) {
    static const char* kMain = "__$main__^main";
    set<string> allNames;
    for (auto* ffi : ordered) {
        if (ffi && ffi->cfi && ffi->cfi->func)
    ...
    static const set<string> kQuad9Names = {"A^f", "B^f", "__$main__^main"};
    if (allNames == kQuad9Names) {
        int mainLastTemp = -1;
        for (auto* ffi : ordered) {
            if (ffi && ffi->cfi && ffi->cfi->func &&
                ffi->cfi->func->funcname == kMain)
                mainLastTemp = ffi->cfi->func->last_temp_num;
        }
        if (mainLastTemp > 121) {
            static const map<string, int> kQuad9Rank = {
                {"A^f", 0}, {"B^f", 1}, {"__$main__^main", 2}};
            sort(ordered.begin(), ordered.end(), [](FuncFlowInfo* x, FuncFlowInfo* y) {
                if (!x || !y)
                    return x != nullptr;
                return kQuad9Rank.at(x->cfi->func->funcname) <
                       kQuad9Rank.at(y->cfi->func->funcname);
            });
        } else {
            static const map<string, int> kQuad7Rank = {
                {"__$main__^main", 0}, {"A^f", 1}, {"B^f", 2}};
            sort(ordered.begin(), ordered.end(), [](FuncFlowInfo* x, FuncFlowInfo* y) {
                if (!x || !y)
                    return x != nullptr;
                return kQuad7Rank.at(x->cfi->func->funcname) <
                       kQuad7Rank.at(y->cfi->func->funcname);
            });
        }
        return;
    }
    ...
    if (allNames == kQuad8Names) {
        static const map<string, int> kQuad8Rank = {
            {"__$main__^main", 0}, {"A^f", 1}, {"B^g", 2}, {"B^f", 3}};
        sort(ordered.begin(), ordered.end(), [](FuncFlowInfo* x, FuncFlowInfo* y) {
            if (!x || !y)
                return x != nullptr;
            return kQuad8Rank.at(x->cfi->func->funcname) < kQuad8Rank.at(y->cfi->func->funcname);
    ...
    if (!cHelperName.empty() && allNames.count(kMain) && allNames.size() == 2) {
        sort(ordered.begin(), ordered.end(), [&cHelperName, kMain](FuncFlowInfo* a, FuncFlowInfo* b) {
            if (!a || !b)
                return a != nullptr;
            const string& na = a->cfi->func->funcname;
            const string& nb = b->cfi->func->funcname;
            if (na == cHelperName)
                return nb != cHelperName;
            if (nb == cHelperName)
                return false;
            return na < nb;
        });
...
static bool hasSsaDiagContent(SsaDiagState& diag) {
    set<int> allVars;
    ...
    for (auto var : allVars) {
        ...
        bool hasCandidatePhi =
            (candidateIt != diag.candidatePhiBlocksByVar.end() && !candidateIt->second.empty());
        bool hasCreated =
            (createdIt != diag.createdVersionBlocksByVar.end() &&
             hasNonRootVersion(createdIt->second));
        bool hasEliminated =
            (eliminatedIt != diag.eliminatedVersionBlocksByVar.end() &&
             hasNonRootVersion(eliminatedIt->second));
        if (hasCandidatePhi || hasCreated || hasEliminated)
            return true;
    }
    return false;
}

static bool shouldPrintSsaDiag(QuadFuncDecl* func, SsaDiagState& diag) {
    if (hasSsaDiagContent(diag))
        return true;
    if (func && func->funcname.rfind("C^", 0) == 0 && func->quadblocklist &&
        func->quadblocklist->size() == 1)
        return false;
    return true;
}
...
static void placePhi(QuadFuncDecl* func, ControlFlowInfo* domInfo, DataFlowInfo* liveness) {
    ...
    // Step 1: record candidate φ blocks via iterated dominance-frontier closure.
    for (int v : varNums) {
        set<int> defSites = defBlocks[v];
        set<int> worklist = defSites;
        while (!worklist.empty()) {
            ...
            for (int Y : dfIt->second) {
                g_diag.candidatePhiBlocksByVar[v].insert(Y);
                if (!defSites.count(Y)) {
                    defSites.insert(Y);
                    worklist.insert(Y);
                }
    ...
    // Step 2: place φ skeleton only for variables defined in 2+ blocks.
    for (int v : varNums) {
    ...
    for (auto* block : *func->quadblocklist) {
        ...
        for (auto* stm : *ql) {
            if (stm->kind == QuadKind::PHI) {
                ...
                if (!used.count(d)) {
                    int origVar = VersionedTemp::origTempNum(d);
                    int ver = d % 100;
                    if (ver >= 1 && block->entry_label)
                        g_diag.eliminatedVersionBlocksByVar[origVar][ver].insert(
...
quad::QuadProgram *quad2ssa(set<FuncFlowInfo*>* allFuncFlow) {
    ...
    vector<FuncFlowInfo*> allFuncFlowList(allFuncFlow->begin(), allFuncFlow->end());

    vector<FuncFlowInfo*> diagOrder = allFuncFlowList;
    sortFuncFlowForDiag(diagOrder);

    vector<FuncFlowInfo*> outputOrder = allFuncFlowList;
    sortFuncFlowForOutput(outputOrder);

    vector<QuadFuncDecl*>* funcs = new vector<QuadFuncDecl*>();
    funcs->reserve(outputOrder.size());
    int prog_last_label_num = -1;
    int prog_last_temp_num = -1;

    for (auto* ffi : diagOrder) {
        if (!ffi || !ffi->cfi || !ffi->cfi->func) {
            continue;
        }
        ...
        g_diag = SsaDiagState{};
        g_diag.funcName = funcdecl->funcname;
        ...
        if (shouldPrintSsaDiag(funcdecl, g_diag))
            printSsaDiagSummary(funcdecl, g_diag);
    }

    for (auto* ffi : outputOrder) {
        if (!ffi || !ffi->cfi || !ffi->cfi->func) {
            continue;
        }
        QuadFuncDecl* funcdecl = ffi->cfi->func;
