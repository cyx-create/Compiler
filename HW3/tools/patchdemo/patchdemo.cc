...
#include "tr_exp.hh" // IWYU pragma: keep
#include "tree2xml.hh" // IWYU pragma: keep
...
tree::Program* generate_a_testIR(Temp_map *tm) {
    ...
    //prepare the external callw vector<tree::Stm*>();
    tree::ExtCall *call = new tree::ExtCall(tree::Type::INT, "putchar", new vector<tree::Exp*>(1, var_x));
    ...
    sl->push_back(new tree::Move(var_x, new tree::Const(2))); // x = 2;
    ...
    return new tree::Program(new vector<tree::FuncDecl*>(1,
            new tree::FuncDecl("__$main__main", nullptr, 
                new tree::Seq(sl),
