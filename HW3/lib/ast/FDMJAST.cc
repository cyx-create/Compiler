    ...
    string ASTKind_string(ASTKind kind) {
        switch (kind) {
            ...
            case ASTKind::NewArray: return "NewArray";
            case ASTKind::NewObject: return "NewObject";
    ...
    PutArray* PutArray::clone() {
        Exp *n_clone = (this->n)? static_cast<Exp*>(n->clone()): nullptr;
        Exp *arr_clone = (this->arr)? static_cast<Exp*>(arr->clone()): nullptr;
        return new PutArray(pos->clone(), n_clone, arr_clone);
    }
    ...
    NewArray* NewArray::clone() {
        Exp *s = (this->size)? static_cast<Exp*>(size->clone()): nullptr;
        return new NewArray(pos->clone(), s);
    }

    NewObject* NewObject::clone() {
        IdExp *i = (this->id)? static_cast<IdExp*>(id->clone()): nullptr;
        return new NewObject(pos->clone(), i);
    }
