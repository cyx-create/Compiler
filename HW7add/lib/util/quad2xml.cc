...
XMLElement* Quad2XML::createElementWithKind(const char* elemName, QuadKind kind) {
    ...
    switch (kind) {
        ...
        case QuadKind::PTR_CALC: elem->SetAttribute("kind", "PTR_CALC"); break;
    }
    
    return elem;
}

void Quad2XML::addTempAttribute(XMLElement* elem, const char* attrName, QuadTemp* temp) {
    if (temp) {
        elem->SetAttribute(attrName, temp->temp->name());
        elem->SetAttribute("temp_type", quadTypeToString(temp->type).c_str());
    }
}
...
void Quad2XML::addQuadTermElement(XMLElement* parent, const char* elemName, QuadTerm* term) {
    ...
    switch (term->kind) {
        case QuadTermKind::TEMP: {
            ...
            QuadTemp* temp = std::get<QuadTemp*>(term->term);
            if (temp) {
                termElem->SetAttribute("id", temp->temp->name());
                termElem->SetAttribute("type", quadTypeToString(temp->type).c_str());
            }
            break;
        }
...
void Quad2XML::processDefUse(XMLElement* elem, set<Temp*>* def, set<Temp*>* use) {
    ...
    if (def && !def->empty()) {
        ...
        for (auto temp : *def) {
            ...
            tempElem->SetAttribute("id", to_string(temp->num).c_str());
...
void Quad2XML::visit(QuadProgram* program) {
    XMLElement* root = createElementWithKind("Program", QuadKind::PROGRAM);
    root->SetAttribute("last_label_num", program->last_label_num);
    root->SetAttribute("last_temp_num", program->last_temp_num);
...
void Quad2XML::visit(QuadPhi* phi) {
    ...
    addTempAttribute(phiElem, "dst", phi->temp_exp);
    ...
    if (phi->args && !phi->args->empty()) {
        ...
        for (auto& arg : *phi->args) {
            XMLElement* argElem = doc->NewElement("arg");
            if (arg.first) {
                // Use str() to get "t" + number format, not name() which returns int
                argElem->SetAttribute("temp", arg.first->str().c_str());
            }
            if (arg.second) {
                // Use str() to get "L" + number format
                argElem->SetAttribute("label", arg.second->str().c_str());
            }
            argsElem->InsertEndChild(argElem);
        }
...
void Quad2XML::visit(QuadReturn* returnQuad) {
    ...
    // Add return value
    if (returnQuad->exp) {
        addQuadTermElement(returnElem, "value", returnQuad->exp);
    }
...
void Quad2XML::visit(QuadPtrCalc* ptrcalc) {
    XMLElement* ptrcalcElem = createElementWithKind("ptrcalc", QuadKind::PTR_CALC);
    
    // Add destination term
    if (ptrcalc->dst) {
        addQuadTermElement(ptrcalcElem, "dst", ptrcalc->dst);
    }
    
    // Add pointer term (base pointer)
    if (ptrcalc->ptr) {
        addQuadTermElement(ptrcalcElem, "ptr", ptrcalc->ptr);
    }
    
    // Add offset term
    if (ptrcalc->offset) {
        addQuadTermElement(ptrcalcElem, "offset", ptrcalc->offset);
    }
    
    // Add def/use information
    processDefUse(ptrcalcElem, ptrcalc->def, ptrcalc->use);
    
    current->InsertEndChild(ptrcalcElem);
}
