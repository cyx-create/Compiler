...
NewArray* create_newArray(XMLElement* element) {
    ...
    while (ce) {
        Exp *e = create_exp(ce);
        if (size == nullptr) size = e;
        else cerr << "Error: NewArray: More than one expression in NewArray" << endl;
        ce = ce->NextSiblingElement();
    }
    if (size == nullptr) {
        cerr << "Error: NewArray: No size expression in NewArray" << endl;
        return nullptr;
    }
...
NewObject* create_newObject(XMLElement* element) {
    ...
    while (ce) {
        if (string(ce->Name()) == "Id") {
            ...
            id = new IdExp(get_position(ce), string(id_str));
        } else {
            cerr << "Error: NewObject: Unknown element in NewObject" << endl;
        }
        ce = ce->NextSiblingElement();
    }
    if (id == nullptr) {
        cerr << "Error: NewObject: No id element in NewObject" << endl;
        return nullptr;
    }
