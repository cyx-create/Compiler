...
class ControlFlowInfo {
    public:
        ...
        void printEverything(); // print all the information above for debugging
...
// Combined structure holding both control-flow and data-flow info for one function
class FuncFlowInfo {
    public:
        ControlFlowInfo* cfi; // Control flow info (predecessors, successors, dominators, etc.)
        DataFlowInfo* dfi;    // Data flow info (liveness, defs, uses)
        int programLastLabelNum; // optional program-level last label number
        int programLastTempNum;  // optional program-level last temp number
        ...
        ) : cfi(cfi), dfi(dfi), programLastLabelNum(programLastLabelNum), programLastTempNum(programLastTempNum) {}
};
