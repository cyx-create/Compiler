...
// 新加工具函数，判断是不是浅不可变类
bool isImmutableClass(const string& cname) {
    if (cname.length() < 10) return false;
    return cname.substr(cname.length() - 10) == "_Immutable";
}
...
void AST_Semant_Visitor::visit(ClassDecl* node) {
    ...
    if (node->eid) {
        ...
        bool cur_imm = isImmutableClass(className);
        bool par_imm = isImmutableClass(parent);

        if (cur_imm != par_imm) {
            cerr << "Error: Immutable class inheritance violation at ";
            cerr << node->getPos()->print();
            cerr << endl;
            // exit(1);
            mode = -1;
            return;
...
void AST_Semant_Visitor::visit(Assign *node) {
    ...
    // 对Immutable field 检查
    if (auto cv = dynamic_cast<ClassVar*>(node->left)) {

        // 取 obj 的类型
        AST_Semant* obj_sem = semant_map->getSemant(cv->obj);

        if (obj_sem && obj_sem->get_type() == TypeKind::CLASS) {
            string cname = get<string>(obj_sem->get_type_par());

            if (isImmutableClass(cname)) {
                cerr << "Error: Cannot assign to field of immutable class ";
                cerr << cname << " at ";
                cerr << node->getPos()->print();
                cerr << endl;
                // exit(1);
